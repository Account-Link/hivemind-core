...
def test_blob_to_raw_ignores_render_query_string():
    raw = reproduce.blob_to_raw(
        "https://github.com/Account-Link/hivemind-core/blob/"
        "abc123/deploy/phala/docker-compose.core.yaml?image_sha=abc1234"
    )

    assert raw == (
        "https://raw.githubusercontent.com/Account-Link/hivemind-core/"
...
services:
  core:
    image: ghcr.io/account-link/hivemind-core:oldtag
  ingress:
...
""".lstrip()
    ...
    assert "ghcr.io/account-link/hivemind-core:abc1234" in rendered
    assert "ghcr.io/account-link/hivemind-core:oldtag" not in rendered
