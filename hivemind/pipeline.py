...
def _policy_allows_aggregate_outputs(policy: str | None) -> bool:
...
def _scope_preserves_generic_aggregate(
    fn: Callable[[str, list, list[dict]], dict],
) -> bool:
    ...
    return (
        ...
        and first.get("bucket_day") == "2026-01-01"
        and first.get("items") == 42
    )


def _rename_scope_function(source: str) -> str:
    tree = ast.parse(source, mode="exec")
    renamed = False
    for node in tree.body:
        if isinstance(node, ast.FunctionDef) and node.name == "scope":
            node.name = "_policy_scope"
            renamed = True
            break
    if not renamed:
        raise ValueError("scope function not found for aggregate repair")
    ast.fix_missing_locations(tree)
    return ast.unparse(tree)


def _wrap_scope_with_aggregate_preserver(source: str) -> str:
    renamed = _rename_scope_function(source)
    wrapper = r'''
def _hivemind_preserve_aggregate_rows(sql, rows):
    if not rows:
        return {"allow": True, "rows": []}
    sql_text = str(sql).lower()
    sql_tokens = set()
    tokenized_sql = sql_text
    for ch in "(),;=+-*/\n\t\r.'\"[]":
        tokenized_sql = tokenized_sql.replace(ch, " ")
    for token in tokenized_sql.split():
    ...
    sensitive_sql_terms = {
        "id", "user_id", "viewer_id", "email", "phone", "url", "title",
        "description", "content", "body", "message", "token", "secret",
        "password", "api_key", "name", "handle", "username",
    }
    if sql_tokens.intersection(sensitive_sql_terms):
        return None
    raw_markers = {
        "id", "user_id", "viewer_id", "email", "phone", "url", "title",
        "description", "content", "body", "message", "token", "secret",
    ...
    raw_substrings = {
        "email", "phone", "url", "title", "description", "content", "body",
        "message", "token", "secret", "password", "api_key", "name", "handle",
        "username",
    }
    out = []
    for row in rows:
        ...
        for key, value in row.items():
            ...
            if (
                lk in raw_markers
                or lk.endswith("_id")
                or any(term in lk for term in raw_substrings)
            ):
                raw_like = True
        if raw_like:
            continue
        for key, value in row.items():
            ...
            metric = (
                ...
                or lk.startswith("sum_")
                or (aggregate_sql and numeric)
            )
            dimension = (
                lk in (
                    "day", "date", "week", "month", "year", "bucket",
                    "period", "category", "topic", "group", "tag",
                    "hashtag", "role", "status", "type", "kind",
                )
                or lk.endswith("_day")
                or lk.endswith("_date")
                or lk.endswith("_week")
                or lk.endswith("_month")
                or lk.endswith("_year")
                or lk.endswith("_bucket")
                or lk.endswith("_period")
                or lk.endswith("_category")
                or lk.endswith("_topic")
                or lk.endswith("_group")
                or lk.endswith("_tag")
                or lk.endswith("_hashtag")
                or lk.endswith("_role")
                or lk.endswith("_status")
                or lk.endswith("_type")
                or lk.endswith("_kind")
            )
            if metric:
                has_metric = True
...
def scope(sql, params, rows):
    aggregate_result = _hivemind_preserve_aggregate_rows(sql, rows)
    if aggregate_result is not None:
        return aggregate_result
    return _policy_scope(sql, params, rows)
'''
    ...
    async def _run_scope_agent(
        ...
        try:
            ...
            if "scope_fn" in data:
                ...
                if (
                    _policy_allows_aggregate_outputs(getattr(req, "policy", None))
                    and not _scope_preserves_generic_aggregate(fn)
                ):
                    logger.info(
                        "Scope agent over-collapsed generic aggregate rows; "
                        "wrapping scope_fn with aggregate-preserving repair"
                    )
                    source = _wrap_scope_with_aggregate_preserver(source)
                    fn = compile_scope_fn(source)
